#ifndef StReadHFTtrees_hh
#define StReadHFTtrees_hh
...
class StReadHFTtrees {
public:
    StReadHFTtrees();
    virtual ~StReadHFTtrees();
    ...
    void  SetMagnetOnOff(Int_t nMagnetOnOff) {MagnetOnOff = nMagnetOnOff;};
    void  SetSaveTree(Int_t nSaveTree) {SaveTree = nSaveTree;};

private:
    TString outFileName,inputDir,outputDir,inFile,inDir_asciifile,asciiParFile;
    Int_t nEvents, plotmode, anamode, filenumber, SaveTree;
    Long64_t startevent, stopevent;
    Int_t first_event, MagnetOnOff;
    Int_t eSE_ME_Flag;
    Int_t eAnalysisNum;
    Int_t eBeamTimeNum;
    TChain* input;
    TChain* input_B;
    ClassDef(StReadHFTtrees,1)
