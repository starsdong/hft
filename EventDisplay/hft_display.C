...
void hft_display(const double B = 1, const Int_t runnumber = 15081042)
{
...
void process_event(Int_t iEvt)
{
  ...
  // Load verteice/hits
  float vx = t->fVertex[0];
  float vy = t->fVertex[1];
  float vz = t->fVertex[2];
  ...
  cout << "    # of Hits = " << nHits << " \t # of tracks = " << nTracks << endl;
  for (Int_t j = 0; j < nTracks; ++j) {
    //if(fabs(t->fTracks_fDca3D[j])>1.0) continue;
    float ox = t->fTracks_fOriginXDca[j];
    float oy = t->fTracks_fOriginYDca[j];
    float oz = t->fTracks_fOriginZDca[j];
    if(sqrt(ox*ox+oy*oy)>1.0) continue;
    if(fabs(oz)>10.0) continue;
